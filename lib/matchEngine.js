...
function loadData() {
  ...
  // Incremental research-overlay files: small JSON files (id -> partial profile
  // fields) added between full character_profiles.json regenerations, since the
  // ~2.6MB base file is too large to safely edit through GitHub's web UI. Each
  // is optional -- a missing or unreadable overlay just leaves the base profile
  // as-is, so this never breaks matching even if a file is absent.
  const overlayFiles = ['character_profiles_updates.json'];
  for (const fname of overlayFiles) {
    try {
      const overlay = JSON.parse(fs.readFileSync(path.join(dataDir, fname), 'utf8'));
      for (const id of Object.keys(overlay)) {
        if (_profiles[id]) Object.assign(_profiles[id], overlay[id]);
      }
    } catch (e) {
      // overlay file missing/unreadable -- fine, fall back to base profiles
    }
  }
}
...
// ── Closed/closing-institution handling ─────────────────────────────────────
//
// A school whose profile carries institution_status "closed" or "closing" is
// confirmed (via dated, sourced research -- see status_detail/status_sources
// on the profile) to no longer be accepting new students. Those schools are
// excluded from the default recommendation shortlist -- a 9th/10th/11th/12th
// grader has no business being pointed at a school that won't exist by the
// time they'd enroll. They stay in the underlying data (never deleted) so
// they can still surface if a student specifically searches for them by name
// (opts.includeClosedSchools / opts.requestedSchoolIds below), and so the
// status can be periodically rechecked (see status_next_check on the profile)
// in case a "closing" school's plans change or a "closed" listing needs a
// correction.
//
// "distressed" and "merging" are NOT excluded here -- those schools are still
// open and enrolling as of their last verification; they're flagged on the
// profile (institution_status/status_detail) purely so the narrative layer
// can mention the risk, not to hide the school from the shortlist.

function isExcludedForClosure(profile) {
  if (!profile) return false;
  return profile.institution_status === 'closed' || profile.institution_status === 'closing';
}
 ...
 * @param {boolean} [opts.includeClosedSchools=false] - if true, don't filter out closed/closing schools (e.g. a "search all schools" or admin/debug view)
 * @param {Set<number|string>} [opts.requestedSchoolIds] - specific scorecard ids the student explicitly asked about; these are never filtered for closure even if opts.includeClosedSchools is false
...
function buildShortlist(answers, opts) {
  ...
  const requestedIds = opts.requestedSchoolIds || null;
  ...
  for (const school of _schools) {
    ...
    // Skip confirmed closed/closing schools by default -- unless the caller
    // opted in (includeClosedSchools) or the student specifically asked about
    // this exact school (requestedIds).
    const specificallyRequested = requestedIds && (requestedIds.has(school.id) || requestedIds.has(String(school.id)));
    if (!opts.includeClosedSchools && !specificallyRequested && isExcludedForClosure(profile)) continue;
...
function serializeForPrompt(school, profile, score, tier, answers) {
  ...
  return {
    ...
    // Non-closure status flags ("distressed" or "merging") that DON'T exclude
    // a school from the shortlist but should shape how confidently it's
    // recommended -- e.g. the narrative layer should mention a merger/rename
    // in progress, or a real financial-distress risk, rather than presenting
    // the school as a normal safe long-term bet. "closed"/"closing" schools
    // never reach here under default opts (see isExcludedForClosure above).
    institutionStatus: profile && profile.institution_status ? profile.institution_status : null,
    institutionStatusDetail: profile && profile.status_detail ? profile.status_detail : null,
